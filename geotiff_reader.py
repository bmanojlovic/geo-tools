...
if __name__ == "__main__":
    ...
    with open(args.points_file, newline='') as points_file, open(args.csv_output, mode='w') as csv_output:
        ...
        if args.geotiff_file != None:
            ...
            sorted_filenames = human_sorting(os.listdir(args.geotiff_dir))
            for filename in sorted_filenames:
                if re.search('(.*).tif$', filename):
                    cr[filename] = ChelsaReader(os.path.join(args.geotiff_dir,filename),args.raster_band)
                    for band in range(1,cr[filename].rastercount + 1):
                        cr['-'.join([filename,f'band{band}'.format(band)]) ] = ChelsaReader(os.path.join(args.geotiff_dir,filename),band)
